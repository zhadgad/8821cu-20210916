...
#endif
	if (LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
		rtw_cfg80211_new_sta(padapter->rtw_wdev, get_addr2_ptr(pmgmt_frame), &sinfo, GFP_ATOMIC);
	#else // kernel >= 7.1.0
...
#if defined(RTW_USE_CFG80211_STA_EVENT) || defined(COMPAT_KERNEL_RELEASE)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
	rtw_cfg80211_del_sta(padapter->rtw_wdev, da, GFP_ATOMIC);
#else // kernel >= 7.1.0
...
release_plink_ctl:
		...
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
			rtw_cfg80211_new_sta(padapter->rtw_wdev, mac, &sinfo, GFP_ATOMIC);
		#else // kernel >= 7.1.0
